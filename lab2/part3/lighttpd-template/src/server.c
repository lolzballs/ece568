...
ssize_t       fuzz_len;
unsigned char fuzz_buf[1024000 * 1024];
...
static void dump_request(void){
    ...
    while(__AFL_LOOP(1000)) {
        ...
        if (write(socket_desc, buf, len) < 0) {
            perror("write");
            exit(EXIT_FAILURE);
        }
...
static void launch_fuzzer_thread(void){
    ...
    pthread_create(&t, &attr, dump_request, NULL);
}
